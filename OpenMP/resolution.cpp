...
void resolveCollisions(Input* input, int NUM_THREADS)
{

    Particle* particles = input->particles;
    int numParticles = input->N;
    double squareSize = input->L;
    double radius = input->r;
    ...
    #pragma omp parallel num_threads(NUM_THREADS) shared(particles, numParticles, squareSize, radius) private(currentParticle, collidingPartner, collision, destination)
    {
