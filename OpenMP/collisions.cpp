 ...
 */
std::vector<Collision> detectCollisions(Input* input, int NUM_THREADS)
{
    int N = input->N;
    double L = input->L;
    double r = input->r;
    ...
    #pragma omp parallel num_threads(NUM_THREADS) shared(input, N, L, r, particles, allPossibleCollisions)
    {
        vector<Collision> possibleCollisions;
        
        int i = 0;
        #pragma omp for nowait private(i) schedule(dynamic, 3)
        for(i = 0; i < N; i++) 
        {
            for(int j = i + 1; j < N; j++) 
            {
        ...
        #pragma omp critical
        allPossibleCollisions.insert(allPossibleCollisions.end(), possibleCollisions.begin(), possibleCollisions.end());
    }
