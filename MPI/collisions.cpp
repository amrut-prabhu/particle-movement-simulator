 ...
 */
std::vector<Collision> detectCollisions(Config* config, Particle* particles)
{
    int rank;
    ...
    int numPossibleCollisions = config->numPossibleCollisions;

	  vector<Collision> allPossibleCollisions;

    int startIndex = (rank - 1)*chunkSize;
    int endIndex = startIndex + chunkSize - 1;
    for (int k = startIndex; k <= endIndex; k++)
    {
        if (k >= numPossibleCollisions) {
            break;
        }
