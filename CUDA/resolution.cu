...
void resolveCollisions(Particle* particles, int numParticles, double squareSize, double radius)
{

    // Iterate through all particles
    // private variables
    Particle* currentParticle;
    Particle* collidingPartner;
    Collision* collision;
    Vector* destination;
    ...
    for (int i = 0; i < numParticles; i++)
    {
        ...
        if (hasCollision(currentParticle))
        {
            ...
            collision = currentParticle->currentCollision;
            ...
            if (hasCollisionWithParticle(currentParticle))
            {
                ...
                collidingPartner = getCollidingPartner(particles,
        ...
        else
        {
            ...
            * current time step.
            */
            destination = getDestination(currentParticle, 1);
                         ...
                         double radius)
{
    ...
    if (isOutOfBounds(newPosition, squareSize, radius))
    {
        ...
        Pair collisionInfo = identifyWallCollision(collisionPoint, newVelocity,
                                                                         squareSize, radius);
        
        xTravelDistance = collisionInfo.collisionTime*(newVelocity->x);
        yTravelDistance = collisionInfo.collisionTime*(newVelocity->y);
