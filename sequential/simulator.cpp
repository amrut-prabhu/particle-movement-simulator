...
int main() 
{
    ...
    for (int i = 1; i <= input->S; i++) {
        ...
        #ifdef MEASURE_ALL
            simulateStep(input, watch);
        #else
            simulateStep(input, nullptr);
        #endif
 ...
 */
void simulateStep(Input* input, Watch* watch)
{
    // 1. Detect all collisions
    #ifdef MEASURE_ALL
        double time_before_detection = get_wall_time();
    #endif
    std::vector<Collision> possibleCollisions = detectCollisions(input);
    #ifdef MEASURE_ALL
...
void updateParticles(Input* input)
{
