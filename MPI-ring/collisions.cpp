 ...
 */
std::vector<Collision> detectCollisions(Config* config, Particle* lowerParticles, Particle* upperParticles)
{
    ...
    {
        Particle* p1 = lowerParticles + i;

        for (int j = 0; j < config->chunkSize; j++)
        {
            Particle* p2 = upperParticles + j;
            if ((p1->i != p2->i))
            {
                Collision* particleCollision = getParticleCollision(p1, p2, r);
            ...
            {
                Collision* wallCollision = getWallCollision(p1, L, r);
 ...
 */ 
Collision* getParticleCollision(Particle* p1, Particle* p2, int r)
{
    int idx1 = p1->i;
    int idx2 = p2->i;
