 ...
 */
std::vector<Collision> detectType1Collisions(Config* config, Particle* particles, int sizePartition)
{
    double L = config->L;
    double r = config->r;

    vector<Collision> allPossibleCollisions;

    for (int i = 0; i < sizePartition; i++) {
        for (int j = i; j < sizePartition; j++) {
            if (j != i)
            {
                Collision* particleCollision = getParticleCollision(particles, j, i, r);
                if (particleCollision == nullptr) {
                    continue;
                }
                allPossibleCollisions.push_back(*particleCollision);
            ...
            else
            {
                ...
                {
                    allPossibleCollisions.push_back(*wallCollision);
    ...
    return allPossibleCollisions;
}

std::vector<Collision> detectType2Collisions(Config* config, Particle* particles, int sizePartition1, int sizePartition2)
{
    double L = config->L;
    double r = config->r;
    ...
    for (int i = 0; i < sizePartition1; i++) {
        for (int j = sizePartition1; j < sizePartition1 + sizePartition2; j++) {
            Collision* particleCollision = getParticleCollision(particles, j, i, r);
            if (particleCollision == nullptr) {
                continue;
            }
...
void sortAndFilterCollisions(std::vector<Collision> possibleCollisions, Config* config, Particle* particles)
{
    int N = config->N;
